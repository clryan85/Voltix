...
class ConfigStore {
  constructor(userDataDir) {
    ...
    this.file = path.join(userDataDir, 'smart-remote-studio.json');
    this.legacyFile = path.join(path.dirname(userDataDir), ['Web','Remote','Studio'].join(''), ['web','remote','studio'].join('-') + '.json');
    this.state = { keys: {}, tokens: {}, lastDevice: null, manualHosts: [] };
    this.load();
  }
  load() {
    try {
      if (!fs.existsSync(this.file) && fs.existsSync(this.legacyFile)) {
        ensureDir(this.userDataDir);
        fs.copyFileSync(this.legacyFile, this.file);
      }
