...
server.listen(Number(process.env.SMART_REMOTE_STUDIO_PORT || 0), '127.0.0.1', () => { const actual = server.address().port; if (process.env.SRS_PORT_FILE) fs.writeFileSync(process.env.SRS_PORT_FILE, String(actual)); console.log(`SMART_REMOTE_STUDIO_PORT=${actual}`); log('app', `Voltix Remote Studio server listening on 127.0.0.1:${actual}`); });
